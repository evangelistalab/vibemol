...
def main():
    ...
    with run_http_server(ROOT) as url, sync_playwright() as playwright:
        browser = playwright.chromium.launch(headless=True, args=['--use-angle=swiftshader'] if args.software else [])
        try:
            ...
            for look in page.evaluate('() => VibeMolAppearanceLooks.list().filter(look => !look.experimental)'):
                page.evaluate('id => VibeMolAppearanceLooks.apply(id)', look['id'])
                png = page.evaluate('''async () => {
                  await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)));
                  const canvas = document.createElement('canvas'); canvas.width=240; canvas.height=160;
                  canvas.getContext('2d').drawImage(document.querySelector('#canvas'),0,0,240,160);
