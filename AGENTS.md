...
- View and Measure retain the same orbital/cloud graphics; Measure picks atoms through surfaces. Edit alone suppresses those graphics without changing saved layer visibility or settings. Coordinates remain available in Edit, and unfinished cell edits are canceled when leaving Edit. `tests/e2e/mode_consistency.py` verifies these behaviors using the existing interface.
...
- View actions include `COM → Origin`, principal-axis alignment, and `+X/+Y/+Z` camera presets; `+X` places positive Y to the right and positive Z up. Shortcut `R` shifts active molecule center of mass to origin.
