...
- Appearance controls include a `Shadows` toggle for atoms, bonds, and isosurfaces casting and receiving shadows together, enabled by default in every built-in preset and at startup. Explicit saved look/session choices are preserved. It preserves the `molecule.feature.shadows` preset key and reuses existing surface geometry. Surface shadows fade with opacity using filtered texel coverage. The key shadow camera fits visible mesh bounds; WBOIT shares one complete shadow map across color passes, and alpha/beta split view builds a separate map per pane. Clouds retain their existing behavior. `tests/e2e/surface_shadows.py` runs through `looks.py` and verifies rendered cross-shadows, transparency/fallback, split isolation, visibility, and sessions.
