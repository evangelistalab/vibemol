...
- Properties consolidation is staged, pending the user's layout and material-scope decisions. `docs/experiments/workbench/inspector-consolidation.md` records the audit and remaining work. Workbench Studio surface mirrors currently share selected-layer scope; the ordinary launch keeps default/override scope. Saved Looks now appear and apply through both preset menus. Material disclosure derives from the shader contract and uses a collapsed Material channels section. Workbench axes/focus live in Camera and simulation boxes use nullable per-layer `showBox`, retained by sessions and excluded from Looks. `tests/e2e/appearance_controls.py` covers these changes.
