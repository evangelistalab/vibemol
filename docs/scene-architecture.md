...
**Scenes → +** offers **Create empty scene** and **From file...**, including at empty startup. Empty scenes reuse the editor's zero-atom molecule record, open in Edit mode, and preserve earlier scenes. Their molecule and Orbitals group are part of the normal scene/source model and persist in complete sessions. **From file...** accepts XYZ, Cube (including Cub/2ccube), and Molden files and passes `newScenes: true` through the shared loader. This skips matching against existing scenes, so loading the same molecule again creates a new scene. Files with matching geometry within one selection still form one scene. The Orbitals group's **Add cube file...** action continues to add matching cube layers to its target scene. Canceled or invalid imports leave the workspace unchanged.
