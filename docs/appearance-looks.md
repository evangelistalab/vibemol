...
- **Glossy finish:** available for all three material targets in the same Finish menu. It restores the original solid Glossy material (roughness `0.045`, metalness `0.03`, clearcoat `1`, coat roughness `0.015`, reflectivity `0.85`). Geometry, lighting, colors, and opacity are preserved. Exact values survive saved materials, looks, and sessions; the legacy `surface.materialPreset: 'glossy'` key is also supported.
