...
**Appearance → Preset** selects **Basic, Classic, Ink, Kit, Opal, Porcelain, and Toon** in alphabetical order, plus your saved looks. The **Style Studio** button directly below opens the floating editor. Choose from rendered preview cards or the preset menu, edit Geometry, Material, and Lighting & contours, and save or share your presets in the same window. The sidebar and Studio selections stay synchronized. Each preset is a complete recipe with explicit display geometry, materials, colors, lighting, and effects. Number keys are reserved for editing. Geometry and material remain independent: Kit connectors can use any material.
...
Looks preserve coordinates, topology, camera pose/projection, visibility, selections, playback, orbital isovalues, Auto-iso, phase mapping, and surface/cloud mode. Depth of field is currently an exception: its focus and blur settings are stored and reapplied by looks. Surface colors and opacity are captured from a representative orbital and applied to every surface layer. The [scope audit](appearance-audit.md#style-studio-scope-audit) describes these remaining mismatches and proposed boundaries. Complete sessions and autosave/recovery preserve the material descriptors and light rig, plus per-orbital colors and scientific settings.
