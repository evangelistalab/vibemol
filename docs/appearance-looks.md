...
- **Material:** one menu and one set of properties for all atoms, bonds, and surfaces, across every loaded molecule and all hidden/deferred orbitals. There are no target tabs, linking controls, or separate Finish/Shading menus. Emissive, Satin, Lacquer, Metal, Gel, and Ceramic reproduce the material recipes from `main` across all three targets. Polished, Matte, Enamel, Classic smooth, and Toon are also available. Only applicable controls appear. Adjusting a property marks the material Custom. Element and orbital phase colors remain distinct.
