...
The faster span picker exposed a separate correctness problem: atom-first interaction handlers selected atoms behind foreground bonds. Dashed metal bonds had no mesh raycast, and the span fallback ranked candidates by screen distance without their depth or actual dash gaps. A minimal three-atom fixture reproduced the report: the pointer ray reached a dash at distance 7.931 Å and an atom at 9.232 Å, yet the atom was selected.

Atom and bond queries now share one cached result for the nearest physical intersection. Dashed bonds first test one conservative bounding box per logical bond, then test the actual cylinder triangles only for instances in candidate boxes. This preserves rendered caps, facets, gaps, transforms, and camera clipping without scanning all 27,000 dash meshes. A 5,000-instance unit fixture requires just five triangle raycasts. Screen-distance tolerance is used only when neither an atom nor a bond occupies the pointer; its depth is expressed in the same ray-distance units as physical hits.

Selected-atom drag/context tolerance, sticky bond-center hints, and hover overlays cannot override the physical target. Bond-section classification also converts world-space intersections into the bond group's coordinates so scene shifts preserve the intended interaction region. The browser regression covers solid and dashed metal bonds, Classic cylinders, Kit curves, atoms visible through dash gaps, overlapping bonds, both camera projections, scene shifts, and live depth changes with a stationary pointer.

The same 2,000-atom hardware benchmark after this correction measured **28.1 ms** for the first pointer move, **8.6 ms** median stationary-hover CPU time (10.3 ms at the 95th percentile), and **6.1 ms** to enter Edit. Shadows and camera-depth fitting were enabled; there were no page errors. These results remain within the previous fix's timing range. Raw profiles and measurements are in `/private/tmp/vibemol-occlusion/benchmark/`. All 311 unit tests, the foreground-picking browser regression, full editing smoke test, camera-depth regression, and Workbench consistency regression passed.
