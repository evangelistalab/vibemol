...
1. **One visible tools bar.** It shows inspectors relevant to the active file. Opening Coordinates keeps Orbitals available. Theme and sidebar controls remain accessible in the same bar.
...
7. **A small-screen version.** Windows share one tabbed bottom panel when there is insufficient horizontal room. Desktop docking preferences are retained when returning to a larger window.

Dock separators support arrow keys, with Shift for fine adjustment. Tabs support Left/Right/Home/End. Window menus and Focus have explicit buttons; no new global shortcuts compete with molecular editing. Escape inside a window closes that window, while text controls retain their editing behavior.

## What belongs where

| Interface role | Prototype behavior | Reason |
| --- | --- | --- |
| Orbitals, Coordinates, Style Studio, View actions, View, Trajectory, Frequencies, Spinor info | Tabbed right/bottom docks, or floating windows | These remain useful while inspecting the molecule. |
| Build, Symmetry, selection tools, bond/coordination menus, placement operators | Existing movable contextual interfaces | Their geometry and gesture workflows need a dedicated edit-workspace study before changing their ownership. |
...
The broader edit-mode smoke test currently times out at the hover-delete assertion in `tests/e2e/smoke.py:1810`. The same test reproduces that failure on unchanged `main` (`a894f57`) with the same local Chromium/SwiftShader configuration. This is recorded separately from Workbench validation; it is not a clean full-suite result.
