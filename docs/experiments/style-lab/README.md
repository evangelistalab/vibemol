...
## Unified Basic material study

[Compare the original Basic with three shared-material candidates](basic-material.html): A · Polished uses the original atom material everywhere; B · Balanced adds restrained color fill and a clearcoat; C · Luminous favors orbital brightness. **C · Luminous was selected for Basic** and now supplies its shared material for atoms, bonds, and orbitals. The comparison retains the original reference settings for inspection.

The images use the actual app renderer and the included methane `canonical_4.cube`, at iso ±0.06 and opacity 1. Basic's geometry, colors, lighting, and shadows stay fixed; every candidate shares the same camera within each view. The second view shows the same structure with the orbital visible. The page includes a lazy-loaded, rotatable VibeMol preview and three portable `.material.json` files. Import those through the Material library to test a candidate on other molecules. The live preview uses `appearanceStudy=1` to leave ordinary autosave and recovery alone.

Run `python tools/render_basic_material_study.py` from the repository root to regenerate the eight PNGs, three material files, and `basic-materials/recipes.json`. The script verifies identical camera/geometry/lighting and checks the actual atom, bond, and surface shader parameters. It uses Playwright Chromium with ANGLE Metal on macOS. The recipes file records the full material descriptors, frozen original Basic reference settings, source, isovalue, and capture cameras. Regeneration reads that historical baseline instead of the current Basic recipe.
